...
async function api(path, opts = {}) {
	...
	if (!res.ok) {
		...
		throw new Error(
			`Plexus API ${res.status} on ${path}: ${body.slice(0, 200)}`,
		);
	}
	return res.json();
}
...
export const plexusApi = {
	...
	listErrors(requestId) {
		...
		return api("/v0/management/errors?limit=500")
			...
			.then((rows) =>
				requestId ? rows.filter((r) => r.request_id === requestId) : rows,
			);
	},

	listPerformance() {
		return api("/v0/management/performance").then((res) =>
			camelToSnake(Array.isArray(res) ? res : res.data || []),
		);
	},
