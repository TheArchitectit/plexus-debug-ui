...
export default function RequestTable({
	...
	const allSelected =
		rows.length > 0 && rows.every((r) => selected.has(r.request_id));
					...
					{rows.map((row) => {
						...
						return (
							...
							>
								...
								<td className="px-3 py-2">
									<span
										className={`px-2 py-0.5 rounded text-xs ${row.response_status === "success" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
									>
										{row.response_status}
									</span>
								</td>
								...
								<td className="px-3 py-2 text-xs whitespace-nowrap">
									{formatDate(row.start_time || row.date)}
								</td>
							</tr>
						);
					})}
