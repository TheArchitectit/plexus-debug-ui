...
function ToolCallCard({ tc, index }) {
  ...
  const borderClass = isRetry
    ? (tc.error != null ? 'border-amber-400' : 'border-blue-300')
    : 'border-slate-200';
  const headerBg = isRetry
    ? (tc.error != null ? 'bg-amber-50' : 'bg-blue-50')
    : 'bg-slate-100';

  return (
    <div className={`border ${borderClass} rounded-lg overflow-hidden`}>
      <div className={`${headerBg} px-4 py-2 flex items-center gap-3 flex-wrap`}>
...
function RetryGroup({ groupName, calls }) {
  ...
  const statusBadge = hasRetries
    ? (anySuccess
        ? <span className="px-2 py-0.5 rounded text-xs font-semibold bg-green-200 text-green-800">✓ Success</span>
        : allFailed
          ? <span className="px-2 py-0.5 rounded text-xs font-semibold bg-red-200 text-red-800">✗ All failed</span>
          : <span className="px-2 py-0.5 rounded text-xs font-semibold bg-amber-200 text-amber-800">⚠ Partial</span>)
    : null;
              ...
              <div key={tc.id || i} className="relative">
                ...
                <div
                  className={`absolute w-3 h-3 rounded-full border-2 z-10 ${
                    tc.is_retry
                      ? tc.error != null ? 'border-amber-500 bg-amber-100' : 'border-blue-400 bg-blue-100'
                      : 'border-slate-400 bg-slate-100'
                  }`}
                  style={{ left: '-8px', top: '16px' }}
                />
                {i < calls.length - 1 && (
                  <div className="absolute left-0 top-7 bottom-0 w-0.5 bg-slate-300" style={{ left: '-3px', height: 'calc(100% + 12px)' }} />
                )}
                <div className="ml-4">
